...
const int temp_high_limit = 227; //on a scale of 0-1023
const int temp_low_limit = 203; //on a scale of 0-1023
...
void setNewCoordinate() {
  if(xEmpty == 0){
    ...
    stepperX.setCurrentPosition(xLast*steps_per_rot/(2*3.14*pulley_radius)); //position (in steps) that will become the new zero location
  }
  if(yEmpty == 0){
    stepperY.setCurrentPosition(yLast*steps_per_rot/(2*3.14*pulley_radius)); 
  }
  if(zEmpty == 0){
    stepperZ.setCurrentPosition(zLast*steps_per_rot/(2*3.14*pulley_radius)); 
  }
  if(extEmpty == 0){
    stepperExt.setCurrentPosition(extLast*steps_per_rot/(2*3.14*pulley_radius)); 
  }
}
...
void G01_Command() {
  ...
  xSteps = round(x/(pulley_radius*step_precision)); //in absolute mode, so using x and not dX
  ySteps = round(y/(pulley_radius*step_precision));
  zSteps = round(z*(1/thread_lead)*steps_per_rot);
  extSteps = round(ext/(pulley_radius*step_precision));

  //calculate the max speed for the motors
  feed = feed/60.0; //convert to mm/sec, the G-code is in mm/min
  
  if(xEmpty == 0 || yEmpty == 0){ //if there are movements in the x/y axis
...
void runXYExtMotors() {
  ...
  if(extEmpty == 0){
    stepperX.moveTo(extSteps);
    stepperX.setSpeed(Vext);
  }
